...
**New file:** `src/compiler/lexer/include/lexer/token.hpp`
...
**New file:** `src/compiler/lexer/include/lexer/lexer.hpp`

Define class `Lexer` with:
...
**New file:** `src/compiler/lexer/lexer.cpp`
...
- `ClassifyToken()`: Check in order:
  1. Keywords ("let", "print", "def", "main")
  2. Operators (+, -, *, /)
  3. Symbols (=, =>)
  4. Hex literals (starts with "0x")
  5. Parameter identifiers (starts with "_")
  6. Regular identifiers (starts with letter)
...
**New file:** `src/unit_tests/lexer/lexer_tests.cpp`
...
**Modify:** `src/compiler/statement/include/statement/expression.hpp`

Add 6 new expression classes (keep existing `Expression`, `FunctionHeadExpression`, `LiteralExpression`):

1. **HexLiteralExpression**
...
**Modify:** `src/compiler/statement/expression.cpp`
...
**Modify:** `src/unit_tests/expression/expression_tests.cpp`
...
**Modify:** `src/compiler/statement/include/statement/statement.hpp`

Add 3 new statement classes:
...
**Modify:** `src/compiler/statement/statement.cpp`
...
**Modify:** `src/unit_tests/statement/statement_tests.cpp`
...
**Modify:** `src/compiler/parser/parser.cpp`
...
**Modify:** `src/compiler/parser/include/parser/parser.hpp`

Add private method declarations for all the parsing helpers above.

### 4.3 Update Parser Tests

**Modify:** `src/unit_tests/parser/parser_tests.cpp`
...
**New file:** `src/compiler/runtime/runtime_helpers.hpp`

Define helper functions (as inline or header-only):
...
**Modify:** `src/compiler/compiler/compiler.cpp`

Update `kMainFunctionSnippet`:
...
**Modify:** `src/unit_tests/compiler/compiler_tests.cpp`
...
1. `src/compiler/lexer/include/lexer/token.hpp` - Token definitions
2. `src/compiler/lexer/include/lexer/lexer.hpp` - Lexer interface
3. `src/compiler/lexer/lexer.cpp` - Lexer implementation
4. `src/unit_tests/lexer/lexer_tests.cpp` - Lexer tests
5. `src/compiler/runtime/runtime_helpers.hpp` - Runtime helpers
6. `src/compiler/lexer/CMakeLists.txt` - Build config for lexer

### Modified Files (7 files)

1. `src/compiler/statement/include/statement/expression.hpp` - Add 6 new expression classes
2. `src/compiler/statement/expression.cpp` - Implement new expressions, add ParsePolishExpression
3. `src/compiler/statement/include/statement/statement.hpp` - Add 3 new statement classes
4. `src/compiler/statement/statement.cpp` - Implement new statements, update PrintStatement
5. `src/compiler/parser/include/parser/parser.hpp` - Add parsing helper methods
6. `src/compiler/parser/parser.cpp` - Refactor to use Lexer, implement statement parsers
7. `src/compiler/compiler/compiler.cpp` - Update template, separate global/main code

### Test Files (4 files)

1. `src/unit_tests/lexer/lexer_tests.cpp` - New
2. `src/unit_tests/expression/expression_tests.cpp` - Expand
3. `src/unit_tests/statement/statement_tests.cpp` - Expand
4. `src/unit_tests/parser/parser_tests.cpp` - Expand
